...
<body>
        ...
        <div class="login-form">
            ...
            <form id="verificationForm" method="POST" action="{{ route('send-otp') }}">
                ...
                <table>
                    ...
                    <tr>
                        <td></td>
                        <td><button class="btn-login" id="loginBtn" type="submit">Verifikasi</button>
                            <p id="sending" style="display: none; margin-top:15px;">Mengirim...</p>
                        </td>
                    </tr>
                </table>
            </form>
    ...
    <script>
        document.getElementById('telepon').addEventListener('input', function (event) {
            this.value = this.value.replace(/[^0-9]/g, ''); // Hanya memperbolehkan angka 0-9
        });
        document.getElementById('telepon').addEventListener('keypress', function (event) {
            if (!/\d/.test(event.key)) {
                event.preventDefault(); // Mencegah input selain angka
            }
        });
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            form.addEventListener("submit", function (e) {
                ...
                fetch("/send-otp", {
                    ...
                    body: JSON.stringify({ email: email, no_hp: telepon }),
                })
