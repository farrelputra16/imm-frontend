...
class TeamController extends Controller
{
    ...
    public function updateTeam(Request $request, $id)
    {
        ...
        $validated = $request->validate([
            ...
            'photo' => 'nullable|url', // Pastikan ini sesuai dengan format yang Anda inginkan
        ]);

        // Update data team
        $team->name = $validated['name'];
        $team->position = $validated['position'];
        $team->photo = $validated['photo'] ?? $team->photo; // Tetap menggunakan foto lama jika tidak ada foto baru
        $team->save();
